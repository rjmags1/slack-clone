...
public class ChannelStoreTests2 : StoreTest
{
    private readonly ChannelStore _channelStore;
    ...
    )
    {
        ...
        _channelStore = new ChannelStore(DbContext);
        _userId = GetUserId();
    }
    ...
    {
        Assert.True(DbContext.Channels.Count() > 0);
        Assert.True(DbContext.Channels.Where(c => c.Private).Count() > 0);
        Assert.True(DbContext.ChannelMembers.Count() > 0);
        Assert.True(DbContext.ChannelMessages.Count() > 0);
        Assert.True(DbContext.ChannelMessageLaterFlags.Count() > 0);
        Assert.True(DbContext.ChannelMessageReactions.Count() > 0);
    }

    [Fact]
    public async void LoadChannelMessages_ShouldWork()
    {
        var channelId = GetChannelIdContainingUser(_userId);
        (var afterId, var totalMessages) =
            GetMostRecentChannelMessageTotalChannelMessages(channelId);
        ...
        (var messages1, var lastPage1) =
            await _channelStore.LoadChannelMessages(channelId, dbCols, first);
    ...
    public async void LoadChannelMembers_ShouldWork()
    {
        ...
        (var afterId, var totalMembers) =
            GetFirstAlphaChannelMemberTotalChannelMembers(channelId);
    ...
    public async void LoadChannels_ShouldWork()
    {
        ...
        var workspaceId = GetWorkspaceIdContainingUser(_userId);
        (var afterId, var totalChannels) = GetFirstAlphaChannelTotalChannels(
            workspaceId
        );

        int first = totalChannels - 1;
        ...
        Assert.False(lastPage1);
        var ordered1 = channels1.OrderBy(c => c.Name);
        Assert.Equal(ordered1, channels1);
        Assert.True(lastPage2);
        var ordered2 = channels2.OrderBy(c => c.Name);
        Assert.Equal(ordered2, channels2);
    }
}
